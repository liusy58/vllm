...
Tensor, pipeline and expert parallelism are supported; data parallelism and
EPLB are rejected at launch. The daemon launches one rank per TP×PP worker,
using the same TP-inner rank layout as the engine. With DP=1 the expert
parallel size equals the tensor parallel size.

For multi-node runs, launch one launcher per node with a shared rendezvous so
the global TP×PP group forms across nodes (CUDA IPC handles are node-local, so
each node serves only its local GPUs' shards). Reuse the same
...
The global rank of local GPU ``i`` on node ``r`` is
``r * (tp_size * pp_size // nnodes) + i``, matching vLLM's contiguous per-node
rank assignment, so each engine worker maps its shard from the daemon on its
own node.
"""
...
class WeightCacheDaemon:
    ...
    def load_model(self) -> None:
        ...
        pp_size = self.cache_config.pp_size
    ...
    def serve_forever(self, ready_callback: Callable[[], None] | None = None) -> None:
        ...
        server.listen()
        logger.info(
            "Weight cache daemon rank %d serving on %s", self.global_rank, socket_path
        )
    ...
    def _handle_release(self, conn: socket.socket) -> None:
        ...
        logger.info(
            "Weight cache daemon rank %d released cached weights", self.global_rank
        )
...
def _run_daemon(
    ...
    daemon = WeightCacheDaemon(
        vllm_config, global_rank, local_rank, distributed_init_method, socket_dir
    )
    daemon.load_model()
    daemon.serve_forever(ready_callback=lambda: ready_queue.put(global_rank))


def _reject_unsupported_parallelism(parallel_config: ParallelConfig) -> None:
    """Reject parallelism modes other than tensor/pipeline/expert parallelism."""
    if parallel_config.data_parallel_size > 1:
        raise ValueError(
            "The weight cache daemon only supports tensor, pipeline and expert "
            "parallelism; data parallelism is not supported"
        )
    if parallel_config.enable_eplb:
        raise ValueError(
            "The weight cache daemon does not support EPLB: expert load "
            "balancing rearranges experts in GPU memory at runtime, which the "
            "static cached layout cannot represent"
        )
...
def main() -> None:
    ...
    parser = FlexibleArgumentParser(
        description="Launch weight cache daemons (one per TP/PP rank)."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Rendezvous port for the daemon's own TP×PP group. Must differ "
        "from the engine's --master-port (the daemon holds its group open "
        "while serving) and match across nodes. Required when --nnodes > 1; "
        "defaults to a free port for single-node.",
    )
    ...
    if world_size % nnodes != 0:
        raise ValueError(
            f"tensor-parallel-size * pipeline-parallel-size ({world_size}) must "
            f"be divisible by --nnodes ({nnodes})"
        )
    local_world_size = world_size // nnodes

    # The daemon forms its own TP×PP group and holds it open while serving, so
    # it needs a rendezvous port distinct from the engine's --master-port. All
    ...
    # Global rank of local GPU i on this node; local index == device index.
    global_ranks = [
